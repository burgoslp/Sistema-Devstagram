        ...
        <div class="md:w-7/12">            
            <div  class="shadow p-5 mb-2 bg-white">
                <ul>
                    @if ($user->followings->count())
                        ...
                        @foreach ($user->followings as $following)
                                ...
                                <li class="flex items-center gap-2 mb-3">
                                    ...
                                    <div class="w-1/2">
                                        <a href="{{route('posts.index',$following->username)}}" class="font-bold">{{$following->username}}</a>
                                        ...
                                        @else
                                            <form action="{{route('users.unfollow',$following->username)}}" method="POST">
                                                @method('delete')
                                                @csrf
                    ...
                    @else
                        <p class="text-gray-600 uppercase text-sm text-center font-bold">Todavía no sigues a nadie.</p>
                    @endif
